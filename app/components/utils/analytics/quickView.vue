<template>
	<template v-if="data && !loading">
		<article v-for="statistic in data" :key="statistic.label"
			class="z-10 flex items-center w-full gap-2 p-2 border rounded-lg bg-gray-50 md:gap-3 md:p-3">
			<div class="flex items-center justify-center p-2 text-white rounded-lg shrink-0 md:p-3"
			...
			<div class="flex-1 min-w-0">
				...
				<div class="flex items-center justify-between gap-2">
					<h3 class="text-base font-extrabold text-gray-900 truncate md:text-xl">{{
						useFormatDuration(statistic.value, statistic.format) }}</h3>

					<p :title="`${useFormatDuration(statistic.difference, statistic.format)}`" class="shrink-0">
						<span :class="statistic.positive ? 'text-green-800' : 'text-red-800'"
	...
	<template v-else>
		<article v-for="i in 4" :key="i"
			class="z-10 flex items-center w-full gap-2 p-2 bg-white border rounded-lg md:gap-3 md:p-3 animate-pulse">
