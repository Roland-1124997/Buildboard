<template>
	<field :name="name" v-slot="{ field }">
		<div :name="name" role="search" class="relative w-full">
			...
			<input :disabled="disabled" :value="localSearch" @input="localSearch = ($event.target as HTMLInputElement).value" :id="name" :placeholder type="search" class="w-full p-2 pl-10 text-gray-900 transition border rounded-xl bg-white/80 placeholder:text-gray-400 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-indigo-500/60 focus:border-indigo-500/60 disabled:opacity-60" autocomplete="off" spellcheck="true" role="searchbox" :aria-label="label" />
