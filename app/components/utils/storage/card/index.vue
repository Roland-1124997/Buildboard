	...
	<div class="flex items-start justify-between w-full gap-3">
		...
		<div class="flex flex-shrink-0 select-none gap-x-1">
			<button
				...
				class="flex items-center justify-center transition-colors w-6 h-6 bg-gray-100 rounded-lg group-hover:bg-gray-200 hover:text-white hover:!bg-blue-700 focus:!bg-transparent focus:!text-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-300"
			...
			<button
				...
				class="flex items-center justify-center w-6 h-6 transition-colors bg-gray-100 rounded-lg group-hover:bg-gray-200 hover:text-white hover:!bg-teal-600 focus:!bg-transparent focus:!text-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-300"
				:title="file.published ? 'Bestand is zichtbaar' : 'Bestand is verborgen'">
				<icon :name="file.published ? 'akar-icons:toggle-on' : 'akar-icons:toggle-off'" class="w-[0.85rem] h-[0.85rem]" aria-hidden="true" />
			</button>
			<button
				aria-label="Bestand verwijderen"
				@click="store.remove(file)"
				class="flex items-center justify-center w-6 h-6 transition-colors bg-gray-100 rounded-lg group-hover:bg-gray-200 hover:text-white hover:!bg-red-700 focus:!bg-transparent focus:!text-red-700 focus:outline-none focus:ring-2 focus:ring-red-300"
				title="Bestand verwijderen">
				<icon name="akar-icons:trash-can" class="w-[0.85rem] h-[0.85rem]" aria-hidden="true" />
			</button>
		</div>
	</div>
</template>
