		...
		<div class="relative grid grid-cols-3 gap-2 mt-4 text-sm">
				...
				<div class="grid grid-cols-[repeat(14,minmax(0,1fr))] gap-0.5 md:gap-1">
					<div v-for="value in visibleHistory" :key="value.day" class="w-full aspect-square">
						<button
							...
							class="relative flex items-center justify-center w-full h-full p-1 transition-all duration-200 rounded md:p-2 md:rounded-md hover:scale-105 focus:outline-none focus:ring-2 focus:ring-offset-1"
			...
			<div class="col-span-3 p-3 md:col-span-2 rounded-xl bg-gray-50 group-hover:bg-gray-100">
				...
				<p class="mt-1 text-sm text-gray-600">
					Status:
					<span class="font-semibold capitalize" :class="isOperational(individualStatus.status) ? 'text-emerald-700' : 'text-rose-700'">{{ individualStatus.status }}</span>
				</p>
...
<script setup lang="ts">
	...
	const downtimeIncidents = computed(() => visibleHistory.value.filter((item) => !isOperational(item.status) || item.downtime_duration > 0).length);
	const latestIncident = computed<StatusHistoryItem | null>(() => {
		return [...visibleHistory.value].reverse().find((item) => !isOperational(item.status) || item.downtime_duration > 0) || null;
	});
	...
	const historyCellClass = (status: string, isSelected: boolean) => {
		const base = isOperational(status) ? "bg-emerald-500/90 hover:bg-emerald-500 focus:ring-emerald-400" : "bg-rose-500/90 hover:bg-rose-500 focus:ring-rose-400";

		const additional = isSelected ? (isOperational(status) ? "ring-offset-1 ring-2 ring-emerald-400" : "ring-offset-1 ring-2 ring-rose-400") : "";
