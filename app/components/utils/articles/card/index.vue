<template>
	<div v-for="article in articles" :key="article.id" class="relative z-10 flex flex-col h-full p-0 overflow-hidden transition-all duration-200 bg-white border border-gray-200 shadow-sm rounded-2xl">
		<div class="flex flex-col flex-1 gap-3 p-5 pt-2">
			<div class="flex items-center justify-between gap-2 mt-3">
				...
				<div class="flex items-center justify-between gap-2">
					<NuxtLink :to="`/artikelen/opstellen?edit=${article.id}`" class="flex items-center justify-center w-8 h-8 text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-700" title="Bewerken">
						<icon name="akar-icons:edit" class="w-4 h-4" aria-hidden="true" />
					</NuxtLink>
					<button @click="store.togglePublish(article)" :aria-label="article.published ? 'Gepubliceerd' : 'Concept'" class="flex items-center justify-center w-8 h-8 text-white transition-colors bg-teal-500 rounded-lg hover:bg-teal-600 focus:ring-teal-600" :title="article.published ? 'Gepubliceerd' : 'Concept'">
			...
			<img v-if="article.thumbnail_url" :src="article.thumbnail_url" :alt="`Thumbnail voor ${article.title}`" class="object-cover w-full h-40 mb-2 rounded-lg" />
