<template>
	<button :disabled="(loading && activeType == type) || activeType == type" type="button" @click="setFilter(type)" :class="['flex items-center justify-center gap-2 px-4 text-sm font-medium transition-colors duration-200 border rounded-lg outline-none focus:outline-none focus:ring-2', large ? 'w-full' : 'w-fit', alwaysShowLabel ? 'py-2' : 'py-[0.68rem] md:py-2', getColorClasses(color, filter === type)]" :aria-label="label" :aria-pressed="filter === type">
