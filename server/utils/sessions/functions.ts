...
export const validateAccessToken = (currentSession: Session | Omit<Session, "user">) => {
    if (!currentSession?.access_token) {
        return {
            valid: false,
            error: { message: 'de gebruiker heeft geen valide acces token', status: 401 }
        }
    }
...
export const validateRefreshToken = (currentSession: Session | Omit<Session, "user">) => {
    if (!currentSession?.access_token) {
        return {
            valid: false,
            error: { message: 'De gebruiker heeft geen valide refresh token', status: 401 }
        }
    }
    return { valid: true, error: null }
}
...
export const useGetSession = async (event: H3Event, client: SupabaseClient<Database>, currentSession: Session | Omit<Session, "user"> | null) => {
    ...
    const { data: supabaseUser, error } = await useFetchSupabaseUser(event,
        client,currentSession.access_token!, session_id
    )
...
export const useDeleteSession = async (client: SupabaseClient<Database>, user: SupaBaseUser) => {
    const { error } = await client.rpc("delete_sessions_by_id", { p_session_id: user.current_session_id })
